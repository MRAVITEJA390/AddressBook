...
from schemas import AddressBookCreate, AddressBookUpdate, AddressBookDetail, AddressCoordinates, AddressWithDistance
...
async def get_address_by_id(address_id: int, db: Session = Depends(get_db)):
    ...
    if address is None:
        raise HTTPException(status_code=404, detail=ADDRESS_NOT_FOUND_MSG.format(address_id))

    return address


@router.post('/address_book/{address_id}/', response_model=AddressBookDetail)
async def update_address(address_id: int, address_info: AddressBookUpdate, db: Session = Depends(get_db)):
    address = db.query(AddressBook).filter(AddressBook.id == address_id).first()
    if address is None:
        raise HTTPException(status_code=404, detail=ADDRESS_NOT_FOUND_MSG.format(address_id))
...
@router.post('/addresses/within_distance/{distance}/', response_model=list[AddressWithDistance])
async def get_addresses_within_distance(distance: float, coordinates: AddressCoordinates,
